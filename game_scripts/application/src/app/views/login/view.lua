...
local passwd = {}
passwd.RESOURCE_FILENAME = "login_passwd.json"
passwd.RESOURCE_BINDING = {
	passwd = "passwd",
	txtPasswd = "txtPasswd",
}
local STATUS = {
...
function LoginView:onCreate()
	...
	-- login spine 背景
	local loginSpine = "login/changjing.skel"
	...
	if (APP_CHANNEL =="none" or APP_CHANNEL =="luo") then
		...
		--密码框
		self.passwdWidget = gGameUI:createSimpleView(passwd, self):init()
		local size = self.passwdWidget:getResourceNode():size()
		self.passwdWidget:xy(pos.x - size.width/2, pos.y - size.height/2)
		local passwd = userDefault.getForeverLocalKey("passwd", "", {rawKey = true})
		self.passwdWidget.txtPasswd:setText(passwd)
		self.passwdWidget.txtPasswd:setPlaceHolderColor(ui.COLORS.DISABLED.GRAY)

		if dev.ONLINE_VERSION_LANGUAGE and ONLINE_USER_NAME then
			self.inputWidget.txtAccount:setString(ONLINE_USER_NAME)
			self.passwdWidget.txtPasswd:setString("")
		end

		adapt.oneLinePos(self.passwdWidget.passwd, self.passwdWidget.txtPasswd, cc.p(15, 0))
...
function LoginView:onLoginClick(node, event)
	if self.serverSelected and self.servers then
		...
		if (APP_CHANNEL == "none" or APP_CHANNEL == "luo") then
			...
			userPass = self.passwdWidget.txtPasswd:getString()
			if userName == "" then
				gGameUI:showDialog{content='请输入账号'}
				return
			end
			if userPass == "" then
				gGameUI:showDialog{content='请输入密码'}
				return
			end
			if not dev.ONLINE_VERSION_LANGUAGE then
				userDefault.setForeverLocalKey("account", userName, {rawKey = true})
				userDefault.setForeverLocalKey("passwd", userPass, {rawKey = true})
			end
			--self:onServerLogin(userName)
			gGameApp.net:sendHttpRequest("POST", "http://175.24.163.50:89/api.php?act=login",
			string.format("userName=%s&userPass=%s",userName,userPass),
			cc.XMLHTTPREQUEST_RESPONSE_STRING,
			function(xhr)
				if xhr.status == 200 then
					local res = json.decode(xhr.response)
					if res.code ~= 0 then
						gGameUI:showTip(res.msg)
						return
					end
					self:onServerLogin(res.data.userName)
				else
					gGameUI:showTip("系统异常请 稍后再试")
				end
			end
		)
		else
