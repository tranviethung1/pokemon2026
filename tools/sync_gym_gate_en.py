#!/usr/bin/env python3
"""Sync gym/gate.csv English text columns from the English Lua source."""
...
def sync(args: argparse.Namespace) -> int:
    ...
    stats = {
        "weatherDesc_updates": 0,
        "palce_updates": 0,
        "placeDesc_updates": 0,
        "specialEffDesc_updates": 0,
        "unchanged": 0,
    }
    ...
    for line_number, row in enumerate(rows[3:], start=4):
        ...
        for lua_field, csv_field in field_map.items():
            ...
            if row[index] != source_value:
                row[index] = source_value
                stats[f"{lua_field}_updates"] += 1
                changed = True
    ...
    for key in ("weatherDesc_updates", "palce_updates", "placeDesc_updates", "specialEffDesc_updates", "unchanged"):
