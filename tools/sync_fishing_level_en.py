#!/usr/bin/env python3
"""Sync fishing/level.csv display fields from the English Lua source."""
...
def load_source_entries(path: Path) -> dict[str, str]:
    ...
    for block in iter_top_level_blocks(text, "csv.fishing.level = {"):
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--source",
        type=Path,
        default=Path("game_origin_config/fishing/level.lua"),
    )
    parser.add_argument(
        "--target",
        type=Path,
        default=Path("game_config/fishing/level.csv"),
    )
