#!/usr/bin/env python3
"""Sync fishing/scene.csv display fields from the English Lua source."""
...
def load_source_entries(path: Path) -> dict[str, str]:
    ...
    for block in iter_top_level_blocks(text, "csv.fishing.scene = {"):
...
def sync(args: argparse.Namespace) -> int:
    ...
    for line_number, row in enumerate(rows[3:], start=4):
        ...
        if source_name is None:
            ...
            if has_cjk(row[name_index]):
                unresolved.append((line_number, row_id, row[name_index]))
            continue
    ...
    for key in ("name_en_updates", "name_updates", "unchanged"):
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--source",
        type=Path,
        default=Path("game_origin_config/fishing/scene.lua"),
    )
    parser.add_argument(
        "--target",
        type=Path,
        default=Path("game_config/fishing/scene.csv"),
    )
