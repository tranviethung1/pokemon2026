#!/usr/bin/env python3
"""Sync cross/union_adventure/car.csv text columns from the English Lua source."""
...
def iter_top_level_blocks(text: str):
    marker = "csv.cross.union_adventure.car = {"
...
def load_source_entries(path: Path) -> dict[str, dict[str, str]]:
    ...
    for block in iter_top_level_blocks(text):
        row_id = extract_int_field(block, "id")
        if not row_id:
            continue
        entries[row_id] = {
            "name": extract_string_field(block, "name") or "",
            "produceDesc": extract_string_field(block, "produceDesc") or "",
            "specialDesc": extract_string_field(block, "specialDesc") or "",
        }
...
def sync(args: argparse.Namespace) -> int:
    ...
    header = rows[0]
    columns = ("name", "produceDesc", "specialDesc")
    column_indexes = {column: header.index(column) for column in columns}

    stats = {
        "name_source": 0,
        "name_dictionary": 0,
        "produceDesc_source": 0,
        "produceDesc_dictionary": 0,
        "specialDesc_source": 0,
        "specialDesc_dictionary": 0,
        "unchanged": 0,
    }
    unresolved: list[tuple[int, str, str, str, str]] = []
    ...
    for line_number, row in enumerate(rows[3:], start=4):
        ...
        row_id = row[0]
        source_entry = source_entries.get(row_id, {})
        changed = False

        for column, index in column_indexes.items():
            new_value, origin = choose_value(
                row[index],
                source_entry.get(column),
                dictionary,
                force=args.force,
            )
            if origin:
                row[index] = new_value
                stats[f"{column}_{origin}"] += 1
                changed = True

        if not changed:
            stats["unchanged"] += 1

        if any(has_cjk(row[column_indexes[column]]) for column in columns):
            unresolved.append(
                (
                    line_number,
                    row_id,
                    row[column_indexes["name"]],
                    row[column_indexes["produceDesc"]],
                    row[column_indexes["specialDesc"]],
                )
            )
    ...
    for key in (
        ...
        "name_dictionary",
        "produceDesc_source",
        "produceDesc_dictionary",
        "specialDesc_source",
        "specialDesc_dictionary",
        "unchanged",
    ):
        print(f"{key}={stats[key]}")
    print(f"remaining_cjk_rows={len(unresolved)}")
    for line_number, row_id, name, produce_desc, special_desc in unresolved[:20]:
        print(
            "unresolved "
            f"line={line_number} id={row_id} name={name!r} "
            f"produceDesc={produce_desc!r} specialDesc={special_desc!r}"
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--source",
        type=Path,
        default=Path("game_origin_config/cross/union_adventure/car.lua"),
    )
    parser.add_argument(
        "--target",
        type=Path,
        default=Path("game_config/cross/union_adventure/car.csv"),
    )
    parser.add_argument(
        "--dictionary",
        type=Path,
