...
def isCompatibility(t, castType):
	...
	if t == LUA_STRING and castType == LUA_CSV:
		return True
	# 新增兼容性：如果默认值是字符串，但实际值是容器（map/array），允许
	# 这种情况常见于默认值是空字符串，但实际数据是 {100,101,...} 格式
	if (t == LUA_ARRAY or t == LUA_MAP) and castType == LUA_STRING:
		return True
	# 新增兼容性：如果默认值是容器，但实际值是字符串（可能是空字符串），允许
	if t == LUA_STRING and (castType == LUA_ARRAY or castType == LUA_MAP):
		return True
	# 新增兼容性：如果默认值是数字/布尔，但实际值是字符串（公式表达式如 6*self:star()），允许
	# 这种情况常见于动态计算的属性值
	if t == LUA_STRING and (castType == LUA_NUM or castType == LUA_BOOL):
		return True
	return False
...
def isMap(s):
	...
	if not (s[0] == '{' and s[-1] == '}'):
		return False
	# 检查内容是否包含 = 号（真正的 map 格式）
	# 如果只是 {100,101,102,...} 这种逗号分隔的数字，不是 map
	content = s[1:-1].strip()
	if not content:
		return True  # 空 map {}
	# 检查是否有 key=value 格式
	...
	for i, c in enumerate(content):
		if c in '{<':
			...
			return True  # 找到顶层的 = 号，是 map
	return False  # 没有找到 = 号，不是 map（可能是字符串）
...
def parseCsv(fileName, keys, vars, defs, lines):
	...
	for col in xrange(len(keyList)):
		luaType, defType = 0, 0
		# language.csv 的 text 列特殊处理：强制为字符串类型（因为 {role1} 这种模板会被误判为 Map）
		isLanguageTextCol = fileName.endswith('language.csv') and varList[col] == 'text'
		# 先获取默认值的lua type
		...
		for row in xrange(len(validMat)):
			...
			# language.csv 的 text 列强制使用字符串类型
			if isLanguageTextCol:
				elemType = LUA_STRING
			luaType = max(luaType, elemType)

		# 该列有默认值的情况，严格判断值类型
		# language.csv 的 text 列特殊处理：跳过类型检查（因为 {role1} 这种模板会被误判为 Map）
		isLanguageTextCol = fileName.endswith('language.csv') and varList[col] == 'text'
		if defType != 0 and not isLanguageTextCol:
			for row in xrange(len(validMat)):
				if validMat[row][col] == None:
					continue
				elemType = whatType(validMat[row][col])
...
def makeElem(luaType, strValue):
	...
	if luaType == LUA_NIL:
		...
		# 兼容处理：如果是空字符串，返回空 map
		if strValue == '""""""' or strValue == '':
			ret = LUA_MAP_FUNC({})
		# 兼容处理：如果期望 map 但实际是 array 格式，用 array 解析
		elif isArray(strValue):
			ret = makeArray(strValue)
		else:
