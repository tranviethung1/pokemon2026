...
slot1 = class("ActivityLuckyCat", Dialog)
slot2 = 6
slot3 = 5
slot1.RESOURCE_FILENAME = "activity_lucky_cat.json"
...
function slot1.onCreate(slot0, slot1, slot2)
	slot3 = slot2.view.luckycat_message_rmb
	...
	idlereasy.when(slot0.leftDatas, function (slot0, slot1)
		...
		for slot7, slot8 in pairs(slot1) do
			function slot9(slot0)
				...
				if slot1:size().height < rich.createWithWidth(string.format(gLanguageCsv.congratulationGetDianomd, slot0[1], slot0[2]), slot2:getFontSize(), nil, slot1:size().width - 10):anchorPoint(0, 1):addTo(slot2):size().height then
...
function slot1.rollCb(slot0)
	slot0.have:text(slot0.rmb:read())
	...
	slot0.max:text(slot3.rmbRndMax)
	adapt.oneLinePos(slot0.max, slot0.icon1, cc.p(10, 0), "left")

	slot0.needCost = slot3.rmbCost

	slot0.cost:text(slot3.rmbCost)
	...
	slot0.times:text(slot0.totalTimes - slot2 .. "/" .. slot0.totalTimes)
	adapt.oneLinePos(slot0.timesLabel, slot0.times, cc.p(0, 0), "left")
	...
	if slot3 == 0.1 then
		...
		performWithDelay(slot0, function ()
			...
			if slot0 then
				...
				slot0 = slot0.set
				uv2 = "leftDatas"
				slot2 = slot2.view.luckycat_message_rmb
...
function slot1.onSure(slot0, slot1)
	if slot0.rmb:read() < slot0.needCost and slot0.remainTime > 0 then
		slot0.curState:set(true)
		uiEasy.showDialog("rmb", {
			titleName = gLanguageCsv.luckyCat
		})
	...
	if slot0.totalTimes < #slot0.originData and slot0.remainTime == 0 then
		...
		uiEasy.showDialog("vip", {
			titleName = gLanguageCsv.luckyCat,
			content = {
				gLanguageCsv.luckyMax,
				string.format(gLanguageCsv.commonVipIncrease, gLanguageCsv.luckyCat)
			}
		})
	...
	gGameApp:requestServer("/game/yy/award/get", function (slot0)
		...
		slot3:numberRoll(slot5, slot2, slot7, slot0)
